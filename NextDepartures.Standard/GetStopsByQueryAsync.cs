...
public partial class Feed
{
    ...
    public async Task<List<Stop>> GetStopsByQueryAsync(string search = "", ComparisonType comparison = ComparisonType.Partial, int results = 0)
    {
        try
        {
            var stopsFromStorage = await _dataStorage.GetStopsByQueryAsync(search, comparison);
